...
abstract class AssembleNeoForgeJarTask @Inject constructor() : AbstractAssembleJarTask() {
			...
			FileSystems.newFileSystem(
				...
				FileSystems.newFileSystem(
					...
					AccessWidenerToTransformer.convert(
						runtimeJarFs.getPath("lambdabettergrass.accesswidener"),
	...
	private fun handleNeoJar(fs: FileSystem) {
		val mixinsJson = JsonParser.parseString(Files.readString(fs.getPath("lambdabettergrass.mixins.json")))
			.asJsonObject
		mixinsJson.addProperty("refmap", "lambdabettergrass-refmap.json")
		this.writeString(fs.getPath("lambdabettergrass.mixins.json"), JsonUtils.GSON.toJson(mixinsJson))
	}
